...
let haveIt = [];
const paragrahps = document.querySelector('.paragraphs');
...
function generate(e) {
    ...
    for (let i = 0; i < inputValue; i++) {
        let random = uniqueRandom(inputValue);
        const element = document.createElement('article');
        element.textContent = text[random];
        paragrahps.appendChild(element);
    }
}

function removeOlds() {
    while (paragrahps.firstChild) {
        paragrahps.removeChild(paragrahps.firstChild);
    }
}

function uniqueRandom(maxNr) {
    let length = text.length - 1;
    let random = Math.round(length * Math.random());

    if (haveIt.length > length) haveIt = [];
